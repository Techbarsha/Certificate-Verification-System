...
app.get("/api/certificate/student_data/:certificateId", async (req, res) => {
  ...
  try {
    ...
    const response = {
      ...
      internshipStartDate: certificate.internshipStartDate,
      internshipEndDate: certificate.internshipEndDate,
    };

    res.status(200).json(response);
  } catch (error) {
    console.error("Error searching for certificate:", error);
    res.status(500).json({ error: "An error occurred while searching for the certificate" });
  }
});

// Generate PDF Route
app.get("/api/generate-pdf/:certificateId", async (req, res) => {
  const { certificateId } = req.params;
  ...
  try {
    ...
    doc.text(`Internship Start Date: ${certificate.internshipStartDate}`);
    doc.text(`Internship End Date: ${certificate.internshipEndDate}`);
    doc.end();
